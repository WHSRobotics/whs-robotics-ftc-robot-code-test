#pragma config(Motor,  motorA,          driveLeft,     tmotorNXT, openLoop, encoder)
#pragma config(Motor,  motorC,          driveRight,    tmotorNXT, openLoop, encoder)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

const float INCH_ENCODERVALUE = 114.591559/2.0; //number of encoder values in one inch forward for a 4 inch wheel
...
const float CHANGE_AVG_RATE_SLOW = 2.485;
const float THRESHOLD = 0.1;//0.04;//-0.16; //-.04 //-0.08
const float TIME_INTERVAL = 35.0; //magic numish
  ...
  ClearTimer(T1);
    ...
    if(time1[T1] >= TIME_INTERVAL) //if 100 Msec have passed
    {
      //get both encoder values
	    ...
	    {
	    	PlayTone(300, 1);
	    	leftEncoderError = abs(leftEncoderRate - CHANGE_AVG_RATE_RUN);
	    	rightEncoderError = abs(rightEncoderRate - CHANGE_AVG_RATE_RUN);
	  	}
	  	else if(nMotorRunState[driveLeft] == runStateHoldPosition && nMotorRunState[driveRight] == runStateHoldPosition)
	  	{
	  		PlayTone(440,1);
	  		inHoldPosition = true;
	  		leftEncoderError = abs(leftEncoderRate - CHANGE_AVG_RATE_SLOW);
	    	rightEncoderError = abs(rightEncoderRate - CHANGE_AVG_RATE_SLOW);
	  	}
	  	else if(((nMotorRunState[driveLeft] == runStateRampDown) && inHoldPosition) && ((nMotorRunState[driveRight] == runStateRampDown) && inHoldPosition))
			{
	  		leftEncoderError = abs(leftEncoderRate - CHANGE_AVG_RATE_SLOW);
	    	rightEncoderError = abs(rightEncoderRate - CHANGE_AVG_RATE_SLOW);
			}
