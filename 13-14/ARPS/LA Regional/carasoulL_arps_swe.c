...
task main()
{
	initializeRobot();
	waitForStart();
	StartTask(RampArm); //-Lift the arm enough to clear the ramp
	moveStraight(90.0, 12.0, 50.0);		//-Go forward
	if(SensorValue[IRSensor] == 9)	 		//-Sensing the IR Beacon until it hits the range of 9
	{
	...
	else if(SensorValue[IRSensor] <= 4) //-Crate 1
	{
