...
/*
	int tallyMaxIndex = checkTally();
	 ...
	 if(tallyMaxIndex == 1 || tallyMaxIndex == 2)
	 {
		...
		if(tallyMaxIndex == 1)	//move straight forward to position 1;
		{
			gyroCenterPivot(-45.0,100);
			needArmUp = true; //whil moving, position arm
			moveStraight(6.0, 100);
		}
		else	//move straight forward to position 2;
		{
			needArmUp = true; //while moving, position arm
			moveStraight(6.0,100);
		}
	}
	else //if IR is in position 3 or 4
	{
		//	turn right;
		gyroCenterPivot(45.0,100);
		if(tallyMaxIndex == 3)
		{
			// move straight forward to position 3;
			needArmUp = true; //while moving, position arm
			moveStraight(6.0,100);
		}
		else
