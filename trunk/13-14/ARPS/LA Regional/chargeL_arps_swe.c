...
task main()
{
	...
	moveStraight(140.0, 19.0, 75.0);	     //have the motor go at 100 power
	moveArc(0.0, 90.0, -75.0); //170 degs
	StartTask(ScoreArm);
	moveStraight(180.0, 38.0, 75.0);
	moveStraight(90.0, 12.0, 75.0);
	boxOpen = true;
	StopTask(HoldBox);
	dropTheBlock();
	while(true){}
...
//	stopDriveTrain();
	...
	nMotorEncoderTarget[sweFR] = (1440.0/2.9/PI)*(2.9*PI);//-5 * 1440;
	motor[sweFR] = 15;
	//motor[sweBR] = -15;
	//motor[sweFL] = -15;
	//motor[sweBL] = -15;
	//nMotorEncoder[sweFR] > -(1440.0/2.9/PI/2)a
	while(nMotorEncoder[sweFR] < (1440.0/2.9/PI)*(2.9*PI))
	{
		writeDebugStreamLine("encoder value %d", nMotorEncoder[sweFR]);
	}
	motor[sweFR] = 0;
	//motor[sweBR] = 0;
	//motor[sweFL] = 0;
